...
const SHOP_CACHE_KEY = 'fontec_footer_shop_cache';
const PAGES_CACHE_KEY = 'fontec_footer_pages_cache';
...
// Helper to sanitize HTML entities returned by WP REST API
...
export function Footer() {
  ...
  useEffect(() => {
    ...
    async function fetchFooterData() {
      try {
        // 1. Fetch Categories in background
          ...
          if (res.ok) {
            ...
            if (isMounted) {
              ...
              localStorage.setItem(
                SHOP_CACHE_KEY,
                JSON.stringify({ items: filtered, timestamp: Date.now() })
              );
            }
          }
        });

        // 2. Fetch Pages in background
        const pagesPromise = fetch(`${WP_BASE_URL}/wp-json/wp/v2/pages?per_page=6`).then(async (res) => {
          if (res.ok) {
            ...
            if (isMounted) {
              ...
              localStorage.setItem(
                PAGES_CACHE_KEY,
                JSON.stringify({ items: formattedPages, timestamp: Date.now() })
              );
            }
          }
        });

        await Promise.all([catPromise, pagesPromise]);
      } catch (error) {
          ...
          <div className="lg:col-span-3 flex flex-col justify-between space-y-6">
            <div>
              ...
              <div className="flex items-center gap-2 text-red-100 my-4">
                <a href="#" className="p-2 bg-white/10 rounded-full hover:bg-white hover:text-red-600 transition-all duration-200" aria-label="Facebook">
                  <svg className="w-4 h-4 fill-currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                </a>
                <a href="#" className="p-2 bg-white/10 rounded-full hover:bg-white hover:text-red-600 transition-all duration-200" aria-label="Instagram">
                ...
                <a href="#" className="p-2 bg-white/10 rounded-full hover:bg-white hover:text-red-600 transition-all duration-200" aria-label="YouTube">
                  <svg className="w-4 h-4 fill-currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
                </a>
                <a href="#" className="p-2 bg-white/10 rounded-full hover:bg-white hover:text-red-600 transition-all duration-200" aria-label="LinkedIn">
              ...
              <div className="space-y-2 text-xs sm:text-sm text-red-50 mt-4 font-medium">
                ...
                <div className="flex items-center gap-2.5">
                  <svg className="w-4 h-4 shrink-0 fill-none stroke-currentColor stroke-2 text-white" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
                  <a href="tel:03463994434" className="hover:text-white transition-colors">0346 3994434</a>
                </div>
                <div className="flex items-center gap-2.5">
                  <svg className="w-4 h-4 shrink-0 fill-none stroke-currentColor stroke-2 text-white" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>
            ...
            <div className="pt-2">
              ...
              <form onSubmit={(e) => e.preventDefault()} className="relative flex items-center w-full">
                ...
                >
                  <svg className="w-4 h-4 fill-none stroke-currentColor stroke-2" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
                </button>
              </form>
            </div>
          </div>
