...
export function Footer() {
  ...
  useEffect(() => {
    ...
    async function fetchFooterData() {
      try {
        // Fetch from internal Next.js proxy route to bypass browser CORS limits
        const res = await fetch('/api/footer-data');
        if (res.ok) {
          const { categories, pages } = await res.json();

          const filtered = (categories || [])
            .filter((cat: any) => cat.slug.toLowerCase() !== 'uncategorized')
            .map((cat: any) => ({
